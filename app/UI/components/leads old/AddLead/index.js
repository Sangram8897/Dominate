...
const AddLead = props => {
  ...
  const [_editID, set_editID] = useState('');
  ...
  getData = async () => {
    const data = await props.navigation.state.params.data;
    if (!IsEmpty(data)){
     console.warn('Edit',data)
     set_addlead({
      _firstname: data.name.split(' ')[0],
      _lastname: data.name.split(' ')[1],
      _email: data.email,
      _phone: data.phone,
      _company: data.company,
      _worth: data.worth,
      _address: data.billingAddress,
      _city: data.shippingAddress.city,
      _state: data.shippingAddress.state,
      _pincode: data.shippingAddress.pincode,
      _website: data.shippingAddress.website,
      _about:data.about,
     });
     if(!IsEmpty(data.source)){
      _source.map((item, key) =>
      item._name === data.source
        ? (_source[key].value = !_source[key].value)
        : (_source[key].value = false),
    );
     }
     set_editID(data._id);
     const newArray = [];
     if(!IsEmpty(data.tags)){
      ...
      {newArray.push({_name:item});}
      )
      console.warn('Add',newArray)
     }
  ...
  nextHandler = ()=>{
    ...
    if (_switch._about) {
      ...
      .then((res)=>{
        if(!IsEmpty(_editID))
        {
          dispatch(Actions.UPDATE_LEAD(_editID,res,leadUpdate_SUCCESS));
        }
        else if(_editID==''){
  ...
  leadUpdate_SUCCESS =async ()=>{
    let viewername=await _addlead._firstname+' '+_addlead._lastname
     props.navigation.navigate('ViewLead', {name: viewername})
  };
  leadAdd_SUCCESS = ()=>{
